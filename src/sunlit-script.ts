      ...
      if (e.key === 'Enter') {
        ...
        if (query) {
          ...
          if (quartoSearch) {
            ...
            const searchBtn = quartoSearch.querySelector('button, .aa-SubmitButton') as HTMLElement;
            if (searchBtn) {
              // Set the search query if there's an input
              const quartoInput = quartoSearch.querySelector('input, .aa-Input') as HTMLInputElement;
              if (quartoInput) {
  ...
  searchInput.addEventListener('keypress', (e: KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      if (query) {
        ...
        if (typeof (window as any).performSearch === 'function') {
          (window as any).performSearch(query);
        } else {
...
function searchInCurrentPage(query: string): SearchResult {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    ...
    console.log('Autocomplete library available:', typeof (window as any)['@algolia/autocomplete-js']);
